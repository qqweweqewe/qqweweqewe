<div align="center">
  
<h2> Иван, Rust разработчик <br></h2>
<b>системное программирование | Linux | DevOps</b><br>
копаюсь в инфраструктуре и низкоуровневых штуках. делаю приколы
тут валяются пет-проекты, подробнее у меня [дома](https://qwew.space)

</div>

---

## интересности
- **[qwewginx](https://github.com/qqweweqewe/qwewginx)** - прокси-сервер на rust. начинался как "о прикольно будет", сейчас уже можно погонять локально.
- **[qwewginx-plugin-registry](https://github.com/qqweweqewe/qwewginx-plugin-registry)** — реестр wasm-плагинов для qwewginx (фича в разработке). хранит артефакты, отдаёт по http. sdk для разработки плагинов

---

## основной стек
![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009900?style=flat&logo=NGINX&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat&logo=prometheus&logoColor=white)

---

## leetcode потому что могу
[![leetcode](https://leetcard.jacoblin.cool/qqweweqewe?theme=dark&animation=false)](https://leetcode.com/u/qqweweqewe/)

---

## контакты

- telegram: [t.me/qqweweqewe](https://t.me/qqweweqewe)
- почта: [golubkovivan4@gmail.com](mailto:golubkovivan4@gmail.com)
- портфолио: [qwew.space](https://qwew.space)
    
---

> _«Интеллект — это способность избегать выполнения работы, но так, чтобы она была сделана»_ - Линус Торвальдс
    
    
PS. да я пикми
